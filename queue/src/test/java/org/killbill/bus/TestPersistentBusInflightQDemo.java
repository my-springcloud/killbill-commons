...
public class TestPersistentBusInflightQDemo {
    ...
    public void beforeClass() throws Exception {
        ...
        properties.setProperty("org.killbill.persistent.bus.main.queue.mode", "STICKY_EVENTS");
        ...
        bus = new DefaultPersistentBus(InTransaction.buildDDBI(dataSource), new DefaultClock(), new ConfigurationObjectFactory(properties).buildWithReplacements(PersistentBusConfig.class, ImmutableMap.<String, String>of("instanceName", "main")),
                                       new MetricRegistry(), databaseTransactionNotificationApi);
    }
    ...
    public void testDemo() throws SQLException, PersistentBus.EventBusException {
        ...
        try {
            ...
            connection.commit();
            // 发布事务事件
            databaseTransactionNotificationApi.dispatchNotification(new DatabaseTransactionEvent(DatabaseTransactionEventType.COMMIT));
        } finally {
            if (stmt != null) {
                stmt.close();
            }
        ...
        Assert.assertTrue(handler.waitForCompletion(1, 300000));
    }
